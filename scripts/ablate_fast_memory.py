"""Ablate fast memory on/off for vAGI."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Ablate fast memory on/off.")
...
def main() -> None:
    ...
    for name, memory_slots in [("on", args.memory_slots), ("off", 0)]:
        records = run_vagi_records(
            ...
            config_overrides={"memory_slots": memory_slots},
            use_kv_cache=True,
        )
        variants[name] = {
    ...
    json_path = out_dir / "fast_memory.json"
    ...
    _write_csv(out_dir / "fast_memory.csv", rows)
