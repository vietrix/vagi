"""Ablate world prediction head on/off for vAGI."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Ablate world head on/off.")
...
def main() -> None:
    ...
    for name, use_world in [("on", True), ("off", False)]:
        records = run_vagi_records(
            ...
            config_overrides={"use_world_pred": use_world},
            use_kv_cache=True,
        )
        variants[name] = {
            "config": {"use_world_pred": use_world},
    ...
    json_path = out_dir / "world_head.json"
    ...
    _write_csv(out_dir / "world_head.csv", rows)
